<header id="header-homepage">
	...
	<ul id="header-slider">
		...
		foreach ( $header_slides as $post ) : setup_postdata( $post ); ?>
			<li style="background: linear-gradient( rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4) ), url('<?php print_custom_field('header_image:to_image_src'); ?>') no-repeat center center; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover;">
				<h1><?php the_title(); ?></h1>
				<p><?php the_content(); ?></p>
			</li>
		<?php endforeach;
